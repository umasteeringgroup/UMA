...
public class UmaTexturePrepareWindow : EditorWindow {
    ...
    public UnityEngine.Object textureFolder;
    ...
    void OnGUI()
    {
        ...
        EnforceFolder(ref textureFolder); 

        if (GUILayout.Button("Process Material"))
        {
            Debug.Log("Processing...");
            if (CreateTextureAndMaterial())
            {
                Debug.Log("Success");
            }
            else
        ...
        slotLibrary = EditorGUILayout.ObjectField("Slot Library", slotLibrary, typeof(SlotLibrary), true) as SlotLibrary;
    ...
    private SlotData CreateSlot_Internal()
    {
        ...
        if (slotLibrary != null)
        {
            slotLibrary.AddSlot(GetAssetName(), slot);
        }
        return slot;
    }
    ...
    private OverlayData CreateOverlay()
    {
        ...
        {
            overlayLibrary.AddOverlay(GetAssetName(), overlay);
        }
        return overlay;
    }
    ...
    private bool CreateTextureAndMaterial()
    {
        textures = null;
        ...
        if (res)
        {
            ...
            { 
                AssetDatabase.LoadAssetAtPath(AssetDatabase.GetAssetPath(textureFolder)+"/"+GetAssetFolder()+GetAssetName()+"_diffuse.png", typeof(Texture2D)) as Texture2D,
                AssetDatabase.LoadAssetAtPath(AssetDatabase.GetAssetPath(textureFolder)+"/"+GetAssetFolder()+GetAssetName()+"_normal.png", typeof(Texture2D)) as Texture2D
            };
        }
        return res;
    }
    ...
    private void DropAreaGUI(Rect dropArea)
    {
        ...
        if (evt.type == EventType.DragPerform)
        {
            if (dropArea.Contains(evt.mousePosition))
            {
                ...
                if (receivedMask == 0x0007 && processAutomatically && overlayLibrary != null )
                {
                    Debug.Log("Drag and Drop initiated processing, Creating Overlay...");
                    if (CreateOverlay() != null)
                    {
                        Debug.Log("Success.");
                    }
