...
using UnityEditor;
using UnityEngine;
using UMA.XNodeEditor.Internal;
...
namespace UMA.XNodeEditor {
    public partial class NodeEditorWindow {
        ...
        public static UMA.XNode.Node[] copyBuffer = null;
        ...
        public UMA.XNode.NodePort DraggedOutputPort { get { UMA.XNode.NodePort result = draggedOutput; return result; } }
        /// <summary> Return the Hovered port or null if not exist </summary>
        public UMA.XNode.NodePort HoveredPort { get { UMA.XNode.NodePort result = hoveredPort; return result; } }
        /// <summary> Return the Hovered node or null if not exist </summary>
        public UMA.XNode.Node HoveredNode { get { UMA.XNode.Node result = hoveredNode; return result; } }

        private UMA.XNode.Node hoveredNode = null;
        [NonSerialized] public UMA.XNode.NodePort hoveredPort = null;
        [NonSerialized] private UMA.XNode.NodePort draggedOutput = null;
        [NonSerialized] private UMA.XNode.NodePort draggedOutputTarget = null;
        [NonSerialized] private UMA.XNode.NodePort autoConnectOutput = null;
            ...
            switch (e.type) {
                    ...
                    if (e.button == 0) {
                        ...
                        if (currentActivity == NodeActivity.DragNode) {
                            ...
                            for (int i = 0; i < Selection.objects.Length; i++) {
                                if (Selection.objects[i] is UMA.XNode.Node) {
                                    UMA.XNode.Node node = Selection.objects[i] as UMA.XNode.Node;
                                    ...
                                    if (offset.sqrMagnitude > 0) {
                                        foreach (UMA.XNode.NodePort output in node.Outputs) {
                                            Rect rect;
                                        ...
                                        foreach (UMA.XNode.NodePort input in node.Inputs) {
                                            Rect rect;
                        ...
                        if (IsHoveringPort) {
                            if (hoveredPort.IsOutput) {
                                ...
                                if (hoveredPort.IsConnected) {
                                    UMA.XNode.Node node = hoveredPort.node;
                                    UMA.XNode.NodePort output = hoveredPort.Connection;
                    ...
                    if (e.button == 0) {
                        ...
                        if (IsDraggingPort) {
                            ...
                            if (draggedOutputTarget != null && graphEditor.CanConnect(draggedOutput, draggedOutputTarget)) {
                                UMA.XNode.Node node = draggedOutputTarget.node;
                            ...
                            IEnumerable<UMA.XNode.Node> nodes = Selection.objects.Where(x => x is UMA.XNode.Node).Select(x => x as UMA.XNode.Node);
                            foreach (UMA.XNode.Node node in nodes) EditorUtility.SetDirty(node);
                            if (NodeEditorPreferences.GetSettings().autoSave) AssetDatabase.SaveAssets();
                        } else if (!IsHoveringNode) {
                ...
                case EventType.KeyDown:
                    ...
                    if (e.keyCode == KeyCode.A) {
                        if (Selection.objects.Any(x => graph.nodes.Contains(x as UMA.XNode.Node))) {
                            foreach (UMA.XNode.Node node in graph.nodes) {
                                DeselectNode(node);
                            }
                        } else {
                            foreach (UMA.XNode.Node node in graph.nodes) {
        ...
        public void Home() {
            var nodes = Selection.objects.Where(o => o is UMA.XNode.Node).Cast<UMA.XNode.Node>().ToList();
            if (nodes.Count > 0) {
        ...
        public void RemoveSelectedNodes() {
            ...
            foreach (UnityEngine.Object item in Selection.objects) {
                if (item is UMA.XNode.Node) {
                    UMA.XNode.Node node = item as UMA.XNode.Node;
        ...
        public void RenameSelectedNode() {
            if (Selection.objects.Length == 1 && Selection.activeObject is UMA.XNode.Node) {
                UMA.XNode.Node node = Selection.activeObject as UMA.XNode.Node;
                Vector2 size;
        ...
        public void MoveNodeToTop(UMA.XNode.Node node) {
            int index;
        ...
        public void DuplicateSelectedNodes() {
            ...
            UMA.XNode.Node[] selectedNodes = Selection.objects.Select(x => x as UMA.XNode.Node).Where(x => x != null && x.graph == graph).ToArray();
        ...
        public void CopySelectedNodes() {
            copyBuffer = Selection.objects.Select(x => x as UMA.XNode.Node).Where(x => x != null && x.graph == graph).ToArray();
        }
        ...
        private void InsertDuplicateNodes(UMA.XNode.Node[] nodes, Vector2 topLeft) {
            ...
            Dictionary<UMA.XNode.Node, UMA.XNode.Node> substitutes = new Dictionary<UMA.XNode.Node, UMA.XNode.Node>();
            for (int i = 0; i < nodes.Length; i++) {
                UMA.XNode.Node srcNode = nodes[i];
                if (srcNode == null) continue;

                // Check if user is allowed to add more of given node type
                UMA.XNode.Node.DisallowMultipleNodesAttribute disallowAttrib;
                ...
                UMA.XNode.Node newNode = graphEditor.CopyNode(srcNode);
            ...
            for (int i = 0; i < nodes.Length; i++) {
                UMA.XNode.Node srcNode = nodes[i];
                if (srcNode == null) continue;
                foreach (UMA.XNode.NodePort port in srcNode.Ports) {
                    for (int c = 0; c < port.ConnectionCount; c++) {
                        UMA.XNode.NodePort inputPort = port.direction == UMA.XNode.NodePort.IO.Input ? port : port.GetConnection(c);
                        UMA.XNode.NodePort outputPort = port.direction == UMA.XNode.NodePort.IO.Output ? port : port.GetConnection(c);

                        UMA.XNode.Node newNodeIn, newNodeOut;
        ...
        bool IsHoveringTitle(UMA.XNode.Node node) {
        ...
        public void AutoConnect(UMA.XNode.Node node) {
            if (autoConnectOutput == null) return;

            // Find compatible input port
            UMA.XNode.NodePort inputPort = node.Ports.FirstOrDefault(x => x.IsInput && graphEditor.CanConnect(autoConnectOutput, x));
