...
namespace UMA.XNodeEditor {
    ...
    public partial class NodeEditorWindow {
        ...
        private List<UMA.XNode.Node> culledNodes;
        ...
        void ShowPortContextMenu(UMA.XNode.NodePort hoveredPort) {
            GenericMenu contextMenu = new GenericMenu();
            ...
            if (NodeEditorPreferences.GetSettings().createFilter) {
                ...
                if (hoveredPort.direction == UMA.XNode.NodePort.IO.Input)
                    graphEditor.AddContextMenuItems(contextMenu, hoveredPort.ValueType, UMA.XNode.NodePort.IO.Output);
                else
                    graphEditor.AddContextMenuItems(contextMenu, hoveredPort.ValueType, UMA.XNode.NodePort.IO.Input);
            }
            contextMenu.DropDown(new Rect(Event.current.mousePosition, Vector2.zero));
            if (NodeEditorPreferences.GetSettings().autoSave) AssetDatabase.SaveAssets();
        }
            ...
            foreach (UMA.XNode.Node node in graph.nodes) {
                ...
                foreach (UMA.XNode.NodePort output in node.Outputs) {
                    ...
                    for (int k = 0; k < output.ConnectionCount; k++) {
                        UMA.XNode.NodePort input = output.GetConnection(k);
        ...
        private void DrawNodes() {
            ...
            if (Selection.activeObject != null && Selection.activeObject is UMA.XNode.Node) {
            ...
            List<UMA.XNode.NodePort> removeEntries = new List<UMA.XNode.NodePort>();

            if (e.type == EventType.Layout) culledNodes = new List<UMA.XNode.Node>();
            for (int n = 0; n < graph.nodes.Count; n++) {
                // Skip null nodes. The user could be in the process of renaming scripts, so removing them at this point is not advisable.
                if (graph.nodes[n] == null) continue;
                if (n >= graph.nodes.Count) return;
                UMA.XNode.Node node = graph.nodes[n];
        ...
        private bool ShouldBeCulled(UMA.XNode.Node node) {
