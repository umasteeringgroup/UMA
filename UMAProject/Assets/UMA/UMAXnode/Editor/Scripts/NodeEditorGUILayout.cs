...
namespace UMA.XNodeEditor {
    ...
    public static class NodeEditorGUILayout {
        ...
        public static void PropertyField(SerializedProperty property, GUIContent label, bool includeChildren = true, params GUILayoutOption[] options) {
            ...
            UMA.XNode.Node node = property.serializedObject.targetObject as UMA.XNode.Node;
            UMA.XNode.NodePort port = node.GetPort(property.name);
            PropertyField(property, label, port, includeChildren);
        }

        /// <summary> Make a field for a serialized property. Manual node port override. </summary>
        public static void PropertyField(SerializedProperty property, UMA.XNode.NodePort port, bool includeChildren = true, params GUILayoutOption[] options) {
            PropertyField(property, null, port, includeChildren, options);
        }

        /// <summary> Make a field for a serialized property. Manual node port override. </summary>
        public static void PropertyField(SerializedProperty property, GUIContent label, UMA.XNode.NodePort port, bool includeChildren = true, params GUILayoutOption[] options) {
            if (property == null) throw new NullReferenceException();
            ...
            if (port == null) EditorGUILayout.PropertyField(property, label, includeChildren, GUILayout.MinWidth(30));
                ...
                if (port.direction == UMA.XNode.NodePort.IO.Input) {
                    // Get data from [Input] attribute
                    UMA.XNode.Node.ShowBackingValue showBacking = UMA.XNode.Node.ShowBackingValue.Unconnected;
                    UMA.XNode.Node.InputAttribute inputAttribute;
                    ...
                    bool usePropertyAttributes = dynamicPortList ||
                        showBacking == UMA.XNode.Node.ShowBackingValue.Never ||
                        (showBacking == UMA.XNode.Node.ShowBackingValue.Unconnected && port.IsConnected);
                    ...
                    if (dynamicPortList) {
                        ...
                        UMA.XNode.Node.ConnectionType connectionType = inputAttribute != null ? inputAttribute.connectionType : UMA.XNode.Node.ConnectionType.Multiple;
                        DynamicPortList(property.name, type, property.serializedObject, port.direction, connectionType);
                        return;
                    }
                    switch (showBacking) {
                        case UMA.XNode.Node.ShowBackingValue.Unconnected:
                ...
                } else if (port.direction == UMA.XNode.NodePort.IO.Output) {
                    // Get data from [Output] attribute
                    UMA.XNode.Node.ShowBackingValue showBacking = UMA.XNode.Node.ShowBackingValue.Unconnected;
                    UMA.XNode.Node.OutputAttribute outputAttribute;
                    bool dynamicPortList = false;
                    ...
                    if (dynamicPortList) {
                        Type type = GetType(property);
                        UMA.XNode.Node.ConnectionType connectionType = outputAttribute != null ? outputAttribute.connectionType : UMA.XNode.Node.ConnectionType.Multiple;
                        DynamicPortList(property.name, type, property.serializedObject, port.direction, connectionType);
                        return;
                    }
                    switch (showBacking) {
        ...
        public static void PortField(UMA.XNode.NodePort port, params GUILayoutOption[] options) {
            PortField(null, port, options);
        }

        /// <summary> Make a simple port field. </summary>
        public static void PortField(GUIContent label, UMA.XNode.NodePort port, params GUILayoutOption[] options) {
            if (port == null) return;
        ...
        /// <summary> Make a simple port field. </summary>
        public static void PortField(Vector2 position, UMA.XNode.NodePort port) {
            if (port == null) return;
        ...
        public static void AddPortField(UMA.XNode.NodePort port) {
            if (port == null) return;
            Rect rect = new Rect();
        ...
        public static void PortPair(UMA.XNode.NodePort input, UMA.XNode.NodePort output) {
        ...
        public static bool IsInstancePortListPort(UMA.XNode.NodePort port) {
            return IsDynamicPortListPort(port);
        }

        [Obsolete("Use DynamicPortList instead")]
        public static void InstancePortList(string fieldName, Type type, SerializedObject serializedObject, UMA.XNode.NodePort.IO io, UMA.XNode.Node.ConnectionType connectionType = UMA.XNode.Node.ConnectionType.Multiple, UMA.XNode.Node.TypeConstraint typeConstraint = UMA.XNode.Node.TypeConstraint.None, Action<ReorderableList> onCreation = null) {
        ...
        public static bool IsDynamicPortListPort(UMA.XNode.NodePort port) {
        ...
        public static void DynamicPortList(string fieldName, Type type, SerializedObject serializedObject, UMA.XNode.NodePort.IO io, UMA.XNode.Node.ConnectionType connectionType = UMA.XNode.Node.ConnectionType.Multiple, UMA.XNode.Node.TypeConstraint typeConstraint = UMA.XNode.Node.TypeConstraint.None, Action<ReorderableList> onCreation = null) {
            UMA.XNode.Node node = serializedObject.targetObject as UMA.XNode.Node;
            ...
            var indexedPorts = node.DynamicPorts.Select(x => {
                ...
                return new { index = -1, port = (UMA.XNode.NodePort)null };
            }).Where(x => x.port != null);
            List<UMA.XNode.NodePort> dynamicPorts = indexedPorts.OrderBy(x => x.index).Select(x => x.port).ToList();
        ...
        private static ReorderableList CreateReorderableList(string fieldName, List<UMA.XNode.NodePort> dynamicPorts, SerializedProperty arrayData, Type type, SerializedObject serializedObject, UMA.XNode.NodePort.IO io, UMA.XNode.Node.ConnectionType connectionType, UMA.XNode.Node.TypeConstraint typeConstraint, Action<ReorderableList> onCreation) {
            ...
            list.drawElementCallback =
                (Rect rect, int index, bool isActive, bool isFocused) => {
                    UMA.XNode.NodePort port = node.GetPort(fieldName + " " + index);
                ...
                (ReorderableList rl) => {
                    ...
                    if (rl.index > reorderableListIndex) {
                        for (int i = reorderableListIndex; i < rl.index; ++i) {
                            UMA.XNode.NodePort port = node.GetPort(fieldName + " " + i);
                            UMA.XNode.NodePort nextPort = node.GetPort(fieldName + " " + (i + 1));
            ...
            list.onAddCallback =
                (ReorderableList rl) => {
                    ...
                    if (io == UMA.XNode.NodePort.IO.Output) node.AddDynamicOutput(type, connectionType, UMA.XNode.Node.TypeConstraint.None, newName);
                    ...
                    if (dynamicPorts[index] == null) {
                        ...
                        for (int k = index + 1; k < dynamicPorts.Count(); k++) {
                            for (int j = 0; j < dynamicPorts[k].ConnectionCount; j++) {
                                UMA.XNode.NodePort other = dynamicPorts[k].GetConnection(j);
            ...
            if (hasArrayData) {
                ...
                while (dynamicPortCount < arrayData.arraySize) {
                    ...
                    if (io == UMA.XNode.NodePort.IO.Output) node.AddDynamicOutput(type, connectionType, typeConstraint, newName);
                    else node.AddDynamicInput(type, connectionType, typeConstraint, newName);
                    EditorUtility.SetDirty(node);
                    dynamicPortCount++;
                }
