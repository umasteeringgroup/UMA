...
namespace UMA.XNodeEditor {
    ...
    class NodeEditorAssetModProcessor : UnityEditor.AssetModificationProcessor {
        ...
        private static AssetDeleteResult OnWillDeleteAsset (string path, RemoveAssetOptions options) {
            ...
            if (scriptType == null || (scriptType != typeof (UMA.XNode.Node) && !scriptType.IsSubclassOf (typeof (UMA.XNode.Node)))) return AssetDeleteResult.DidNotDelete;
            ...
            for (int i = 0; i < guids.Length; i++) {
                ...
                for (int k = 0; k < objs.Length; k++) {
                    UMA.XNode.Node node = objs[k] as UMA.XNode.Node;
        ...
        private static void OnReloadEditor () {
            ...
            string[] guids = AssetDatabase.FindAssets ("t:" + typeof (UMA.XNode.NodeGraph));
            for (int i = 0; i < guids.Length; i++) {
                string assetpath = AssetDatabase.GUIDToAssetPath (guids[i]);
                UMA.XNode.NodeGraph graph = AssetDatabase.LoadAssetAtPath (assetpath, typeof (UMA.XNode.NodeGraph)) as UMA.XNode.NodeGraph;
                ...
                for (int u = 0; u < objs.Length; u++) {
                    ...
                    if (!graph.nodes.Contains (objs[u] as UMA.XNode.Node)) graph.nodes.Add(objs[u] as UMA.XNode.Node);
                }
            }
        }
    }
