...
namespace UMA.XNodeEditor {
    ...
    [CustomNodeGraphEditor(typeof(UMA.XNode.NodeGraph))]
    public class NodeGraphEditor : UMA.XNodeEditor.Internal.NodeEditorBase<NodeGraphEditor, NodeGraphEditor.CustomNodeGraphEditorAttribute, UMA.XNode.NodeGraph> 
    {
        ...
        public virtual string GetNodeMenuName(Type type) {
            ...
            UMA.XNode.Node.CreateNodeMenuAttribute attrib;
        ...
        /// </summary>
        public virtual bool CanConnect(UMA.XNode.NodePort output, UMA.XNode.NodePort input) {
        ...
        public virtual void AddContextMenuItems(GenericMenu menu, Type compatibleType = null, UMA.XNode.NodePort.IO direction = UMA.XNode.NodePort.IO.Input) {
            ...
            for (int i = 0; i < nodeTypes.Length; i++) {
                ...
                if (disallowed) menu.AddItem(new GUIContent(path), false, null);
                    ...
                    UMA.XNode.Node node = CreateNode(type, pos);
        ...
        public virtual Gradient GetNoodleGradient(UMA.XNode.NodePort output, UMA.XNode.NodePort input) {
        ...
        /// <param name="output"> The output this noodle comes from. Never null. </param>
        /// <param name="input"> The output this noodle comes from. Can be null if we are dragging the noodle. </param>
        public virtual float GetNoodleThickness(UMA.XNode.NodePort output, UMA.XNode.NodePort input) {
            return NodeEditorPreferences.GetSettings().noodleThickness;
        }

        public virtual NoodlePath GetNoodlePath(UMA.XNode.NodePort output, UMA.XNode.NodePort input) {
            return NodeEditorPreferences.GetSettings().noodlePath;
        }

        public virtual NoodleStroke GetNoodleStroke(UMA.XNode.NodePort output, UMA.XNode.NodePort input) {
            return NodeEditorPreferences.GetSettings().noodleStroke;
        }

        /// <summary> Returned color is used to color ports </summary>
        public virtual Color GetPortColor(UMA.XNode.NodePort port) {
        ...
        /// <returns></returns>
        public virtual GUIStyle GetPortStyle(UMA.XNode.NodePort port) {
        ...
        public virtual Color GetPortBackgroundColor(UMA.XNode.NodePort port) {
            return Color.gray;
        }
        ...
        public virtual string GetPortTooltip(UMA.XNode.NodePort port) {
        ...
        public virtual UMA.XNode.Node CreateNode(Type type, Vector2 position) {
            Undo.RecordObject(target, "Create Node");
            UMA.XNode.Node node = target.AddNode(type);
        ...
        public virtual UMA.XNode.Node CopyNode(UMA.XNode.Node original) {
            Undo.RecordObject(target, "Duplicate Node");
            UMA.XNode.Node node = target.CopyNode(original);
        ...
        public virtual bool CanRemove(UMA.XNode.Node node) {
            // Check graph attributes to see if this node is required
            Type graphType = target.GetType();
            UMA.XNode.NodeGraph.RequireNodeAttribute[] attribs = Array.ConvertAll(
                graphType.GetCustomAttributes(typeof(UMA.XNode.NodeGraph.RequireNodeAttribute), true), x => x as UMA.XNode.NodeGraph.RequireNodeAttribute);
        ...
        public virtual void RemoveNode(UMA.XNode.Node node) {
        ...
        UMA.XNodeEditor.Internal.NodeEditorBase<NodeGraphEditor, NodeGraphEditor.CustomNodeGraphEditorAttribute, UMA.XNode.NodeGraph>.INodeEditorAttrib {
