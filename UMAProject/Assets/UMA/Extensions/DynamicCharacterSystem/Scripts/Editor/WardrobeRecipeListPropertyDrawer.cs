		...
		if (foldoutProp1.isExpanded) {
			...
			for (int i = 0; i < thisRecipesProp.arraySize; i++) {
				...
				if (!recipeIsLive)
				{
					...
					var warningGUIContent = new GUIContent("", thisElement.FindPropertyRelative("_recipeName").stringValue + " was not Live. You can make it live by checking it on in the UMA/UMA Global Library window.");
