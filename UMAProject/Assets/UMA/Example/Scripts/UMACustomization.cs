...
public class UMACustomization : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (Input.GetMouseButtonDown(1))
		{
			...
			{
				// This is pretty ugly, move up the collider?
				Transform tempTransform = hit.collider.transform.parent.parent;

				UMAData newData = tempTransform.GetComponent<UMAData>();
				if (newData != null)
				{
					umaData = null;
					AvatarSetup(newData);
	...
	private void AvatarSetup(UMAData targetUMA)
	{
		umaDynamicAvatar = targetUMA.gameObject.GetComponent<UMADynamicAvatar>();
		
		if (cameraTrack)
		{
			cameraTrack.target = targetUMA.umaRoot.transform;
		}
		
		umaDna = targetUMA.GetDna<UMADnaHumanoid>();
		umaTutorialDna = targetUMA.GetDna<UMADnaTutorial>();
		ReceiveValues();
		umaData = targetUMA;
	}
