...
{	
	...
	private float[] sliderOldValue2;
	private float[] sliderOldValue3;
	...
	private void Start () 
	{
		...
		sliderOldValue = new float[19];
		sliderOldValue2 = new float[18];
		sliderOldValue3 = new float[14];

		InitSliders();

    	int randomResult = Random.Range(0, 2);
		crowdHandle.GenerateOneUMA(randomResult);
		GetUMAData();

		for(int i = 0; i < sliderControlList.Length; i++)
			sliderOldValue[i] = sliderControlList[i].value;

		for(int i = 0; i < sliderControlList2.Length; i++)
			sliderOldValue2[i] = sliderControlList2[i].value;

		for(int i = 0; i < sliderControlList3.Length; i++)
			sliderOldValue3[i] = sliderControlList3[i].value;
    
		InvokeRepeating("UpdateUMA", 0, 0.1f);
	}

	private void GetUMAData()
	{
		UMAData tempUMA = GameObject.Find("UMACrowd").transform.GetChild(0).GetChild(0).gameObject.GetComponent<UMAData>();
		if(tempUMA)
		{
			umaData = tempUMA;
	...
	{	
		if(umaData)
		{
			...
			if(bodyArea == 0)
			{
				for(int i = 0; i < sliderControlList.Length; i++)
				{
					if(sliderControlList[i].value != sliderOldValue[i])
					{
						if(i == 18)
							UpdateUMAShape(true);
						else
							UpdateUMAShape(false);
						sliderOldValue[i] = sliderControlList[i].value;
						break;
					}
			...
			else if(bodyArea == 1)
			{
				for(int i = 0; i < sliderControlList2.Length; i++)
				{
					if(sliderControlList2[i].value != sliderOldValue2[i])
					{
						if(i == 15 || i == 16 || i == 17)
							UpdateUMAShape(true);
						else
			...
			else
			{
				for(int i = 0; i < sliderControlList3.Length; i++)
				{
					if(sliderControlList3[i].value != sliderOldValue3[i])
					{
						if(i == 13)
							UpdateUMAShape(true);
						else
	...
	public void ButtonMale()
	{
		...
		crowdHandle.GenerateOneUMA(0);

		//Invoke("GetUMAData", 0.2f);
	...
	{
		/*charName = panel.FindChild("InputField/Text").GetComponent<Text>().text;
		if(charName != "")
		{
			string savedUMA = umaDnaoidStructure.Pack(umaDna);

			if(!Directory.Exists(Application.dataPath + "\\Characters"))
				Directory.CreateDirectory(Application.dataPath + "\\Characters");

			string path = Application.dataPath + "\\Characters\\" + charName + ".chr";
			System.IO.File.WriteAllText(path, savedUMA);
	...
	private void UpdateUMAShape(bool shape)
	{
		//umaDnaoidBuilder.Update("Char", umaDna, shape, true);
	}
	...
	private void TransferValues()
	{
		...
		{	
			if(bodyArea == 0)
			{
				umaDna.height = (int)sliderControlList[0].value;
	...
	/*private void AddParts(char gender, int color)
	{
		if(gender == 'M')
		{
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Male Eyes 01", color);
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Male Head 01", color);
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Male HeadEars 01", color);
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Male HeadEyes 01", color);
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Male HeadMouth 01", color);
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Male HeadNose 01", color);
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Male Head InnerMouth");
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Male Torso 01", color);
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Male Hands 01", color);
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Male Legs 01", color);
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Male Feet 01", color);
			umaDnaoidStructure.BodyAdd(umaDna, "Female Hair1", 12);

			umaDnaoidStructure.WardrobeAdd(umaDna, "Male Jeans 01", 12);
		}
		else
		{
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Female Eyes 01", color);
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Female Head 01", color);
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Female HeadEars 01", color);
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Female HeadEyes 01", color);
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Female HeadMouth 01", color);
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Female HeadNose 01", color);
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Female Head InnerMouth");
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Female Torso 01", color);
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Female Hands 01", color);
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Female Legs 01", color);
			umaDnaoidStructure.BodyAdd(umaDna, "umaDna Female Feet 01", color);
			umaDnaoidStructure.BodyAdd(umaDna, "Female Hair1", 12);

			umaDnaoidStructure.WardrobeAdd(umaDna, "Female Underwear", 12);
		}
	}*/
}
