...
namespace UMA.HairCards
{
    ...
    public sealed class HairSurfaceFields
    {
        ...
        internal void Prepare(HairGroomEvaluator.SourceMeshReadCache source, HairGroup group)
        {
            ...
            bool same = vertices.Count == positions.Count && normals.Count == ns.Count && triangles.Count == source.SubmeshCount;
        ...
        public float Sample(HairGrowthMap map, HairSurfaceAnchor anchor, float fallback = 1f)
        {
            ...
            float value = map.SampleVertex(a) * bc.x + map.SampleVertex(b) * bc.y + map.SampleVertex(c) * bc.z;
        ...
        public float PaintedDensity(int vertex) => (uint)vertex < growth.Count ? growth[vertex] * density[vertex] : 0f;
        ...
        private void BuildDistances()
        {
            ...
            var queue = new SortedSet<(float distance, int vertex)>();
            void Seed(int v) { if (distances[v] == 0f) return; distances[v] = 0f; queue.Add((0f, v)); }
            for (int i = 0; i < count; i++) if (maxGrowth[canonical[i]] <= 0.001f) Seed(i);
            ...
            while (queue.Count > 0)
            {
                ...
                foreach (int other in adjacency[item.vertex])
                {
                    ...
                    queue.Remove((distances[other], other)); distances[other] = candidate; queue.Add((candidate, other));
                }
            }
