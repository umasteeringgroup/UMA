...
**Overlay Painter Compact View** under **Project Settings > UMA > Editor Settings** defaults on. It opens one floating workspace with **Overlay Painter Layers** and **Overlay Painter Brush** as the left tab group and a dedicated **Scene** view plus **Overlay Painter 2D** as the right tab group. The 40/60 split is resizable, Layers and Scene are initially active, the selected Body target is framed, and window geometry is remembered. Overlay Painter's 3D, Path, and painting toolbar overlays register hidden, become visible only while an Overlay Painter stage is active, and are bound exclusively to that dedicated Scene tab in Compact View. Unity layout restoration is reconciled automatically so they remain hidden in fresh projects and other Scene windows. Use **Layout > Reset Compact View** or **Window > UMA > Reset Overlay Painter Compact View** to restore the arrangement. Disable the setting to open Layers, Brush, and 2D as traditional independent dockable windows and use the existing Scene view. **Overlay Painter Layers** contains the global toolbar and a space-saving tab group that switches between the searchable Paint Target navigator and the combined Layer/Path stack with contextual Properties inspector. **Overlay Painter Brush** owns the brush Asset Shelf together with Brush and Stroke/Projection controls; its toolbar can hide or restore the shelf, and the horizontal divider resizes it. All 13 painting-palette controls live in the native Scene-view **Overlay Painter Toolbar** overlay: Paint, Erase, Blur, Smear, Clone, Dodge, Burn, Normal Touch-up, Plugin Brush, Polygon Fill, UV Island Fill, Path, and help. The former dockable toolbar is no longer opened or offered in the Window/Layout menus; serialized legacy instances close themselves after layouts reload. Every character launch gives startup priority to a logical target whose visible name contains the standalone word **Body** and enables **Isolate** by default for fresh or legacy workspace state; its saved target is the fallback only when no such Body target exists, while an explicit current Isolate choice is retained. Standalone slot launches are unchanged. Workspace selections persist in the avatar recipe state. The 3D Scene-view controls include a full-width **Shutdown Overlay Painter** button that uses the normal Save/Discard/Cancel close flow. Compilation, assembly reload, and Play Mode transitions close the custom stage and immediately hide all Overlay Painter Scene-view toolbars so no inactive toolbar remains behind.

Recoverable import conditions do not interrupt stage opening. A yellow warning icon appears beside each affected logical target, expanded UDIM member, and Texture Set; click it to open the complete code, severity, material, slot scope, and message. Conditions that cannot be associated with reconstructed geometry use the warning icon beside the target search controls. The Scene view also shows a passive notice along its bottom for about ten seconds and fades it during the final 2.5 seconds. Reconstruction failures that make painting unsafe still stop the launch and report the error.
...
- Every Paint, Fill, Path, or Group layer can own one editable grayscale layer mask. Add a black or white mask from the layer-row menu, click its thumbnail to enter **Layer Mask mode**, and paint it in either the 2D or 3D view with the regular brush tools. Press **Escape** to leave Layer Mask mode; Overlay Painter consumes that key in its Layers, Brush, 2D, and dedicated compact Scene views so Unity does not close the painting stage or open the save prompt. If a mask stroke is still active, its painted portion is committed before the mode exits. An active Geometry Fill keeps first priority, so the first Escape cancels that fill mode and a second exits Layer Mask mode. Mask painting is deliberately channel-free and grayscale-only: **Mask Value** selects a scalar from black (hidden) to white (visible), and texture, sprite, material-channel, and OverlayData paint sources are unavailable. Both views display a Layer Mask label; **Solo Mask** previews the effective mask on the model. **Layer Mask Noise** and **Layer Mask Texture Overlay** provide non-destructive texture variation without changing the mask brush source. **Fill Polygon** and **Fill UV Island** are ordinary paint operations on Paint layers and masks. Groups apply opacity, blend mode, and their mask once to the isolated child composite.
- Slot targets: one or more reconstructed UMA slots can be selected together. In 3D, brush application is based on the full world-space footprint rather than only the center ray: every selected slot and UV island intersected by the brush receives a projected stamp, including slots backed by different materials before the cursor center crosses their border. A linked stroke synchronizes its channel sources across every physical member, repairs stale empty Texture-source metadata from the valid stroke source, and refuses to start with a named error if any selected member still lacks a writable raster target. Target-filtered raycasts ignore closer unselected surfaces, the Backfaces setting applies to both hover and projection queries, mouse capture lasts for the complete gesture, and a bounded 0.1-second/16-pixel grace tolerates an isolated seam miss without bridging a broad empty region. Per-triangle slot ownership excludes unselected geometry, and a cached spatial triangle grid keeps footprint queries local. In 2D, the active texture set is painted directly wherever the normalized-UV brush lies; ordinary brush movement and rasterization do not query or clip against the mesh.
...
Stroke kernels copy, compose, and repack only dirty tiles, including each filter's required halo. Every
raw freehand input event queues its already-accepted normal and mirrored contacts, preserves their
original order, and submits them in bounded batches grouped by destination 128-pixel tile. Projected
batch stamps carry their own triangle UVs and boundary ownership, so this optimization does not alter
all-member UDIM queries, slot selection, seam padding, or the shared-edge rules. Compositing,
`TextureChanged`, and preview packing are deferred until that input event finishes, then run once per
dirty target tile instead of once per contact. Paint kernels use synchronized source/destination
ping-pong so the same path remains portable to GLES3, and spline/path
samples use the same bounded GPU path. Capped stroke coverage is allocated as sparse 128-pixel tiles
instead of full-size float textures. When compute shaders are unavailable or a configured memory
budget would be exceeded, the same ordered samples run through the existing compute or sparse CPU
fallback.

Undo/redo stores only touched tiles. A target/history-tile pair is captured immediately before its
first write and only once for the complete stroke, including a rewind/replay; later contacts reuse that
original capture. Before/after pixels are captured through asynchronous GPU readback where supported,
kept in the target's exact graphics format, and losslessly Deflate-compressed. Undo work and memory
therefore scale with painted area rather than total texture size. The **Performance & Memory** panel
exposes rolling preview p95/maximum latency, dirty copy/compose counts, compute/fallback counts, undo
and active-stroke memory, configurable budgets, and a resource-baseline leak check.

For focused painting analysis, enable **Stroke Diagnostics** in **Performance & Memory** and click
**Capture Next Stroke**. Diagnostics are disabled by default and retain no per-stroke samples until a
capture is queued. The resulting copyable report compares early/middle/late input latency and breaks
the stroke into input raycasting, sample projection, spatial contact discovery, per-contact preparation,
history capture, raster submission, compositing, preview binding, and finalization. It also records
normal/mirrored footprint counts, spatial cells and triangle candidates, accepted contacts, raster target
visits, compute/fallback counts, dirty pixels, history/coverage working sets, and stored stroke/contact
state. Capture-thread managed allocations and nested phase allocation totals expose list, dictionary, and
GC pressure. Matching `OverlayPainter.Stroke.*` markers appear in the Unity Profiler. Reported phase time
is main-thread CPU submission time; use the GPU Profiler alongside it to distinguish GPU execution or
queue pressure. Disable **Stroke Diagnostics** at any time to remove its hot-path collection immediately.
...
compact tiles to the host. Compressible tiles remain compressed until their individual upload, then
are released immediately, so multi-channel UDIM generators retain atomic behavior without queuing
every raw 2K/4K output in memory.
...
Twelve production material generators are also included as first-class Plugin layers: **Cloth Texture**,
**Quilt, Embroidery, Perforation & Atlas Scatter**, **Text**, **Fabric Fuzz & Fiber Fray**, **Rust,
Oxidation & Corrosion**, **Surface Noise & Micro Detail**, **Veins & Subdermal Skin**, **Stubble
Maker**, **Scar, Wound & Skin Damage**, **Creature Skin Variation**, and **Combat Scratches & Dents**.
...
**Stubble Maker** creates downward-growing, tapered facial-hair or shaved-head stubble without
baking a base skin color. Length, width, color, density, direction, curvature, random root position,
and per-strand variation are independent. A bounded rectangle or ellipse, rotation, feather, and
optional Control Mask provide placement. Soft cool stubble shadow, shaving redness, rash, pimples,
and pigment spots can be mixed into coordinated Albedo, Roughness, Normal Control, Skin Color Mask,
and Detail Mask outputs. All outputs retain transparent feature alpha. To make a recipe-ready skin
overlay, choose **Export Textures & UMA Assets... > Runtime Overlay (Transparent)**; the exported
`OverlayDataAsset.alphaMask` is built from the authored alpha rather than the underlying skin.
...
Opening the stage starts a temporary session without changing the character, scene, recipe, source overlay, or source textures. After an edit, recovery creates `painter_recovery.asset` and a sibling `painter_recovery Data` folder in the recovery folder configured under **Project Settings > UMA > Editor Settings**. The default folder is `Assets/UMA/Temp`. **Enable Automatic Recovery** defaults on, **Recovery Idle Delay** defaults to 120 seconds after the most recent edit, and **Minimum Save Interval** defaults to 300 seconds between completed background saves. The same scheduler autosaves modified permanent documents. Disable automatic recovery to stop periodic background saves; manual Save and the unsaved-changes close prompt remain available. The next compatible ordinary launch offers Recover, Discard, and Cancel choices. When a saved document was opened explicitly while compatible recovery exists, **Open `<document>`** is the default choice and discards the older recovery; **Recover Instead** is available but never silently replaces the requested document. **Save As** creates a permanent, versioned `TexturePaintDocument` wherever the user chooses below `Assets`, with compressed pixel blobs in a sibling `<Document Name> Data` folder. Double-click a compatible document asset in the Project window to open it directly, or use **File > Load Document** in an active Overlay Painter stage. Later recovery and document saves reuse unchanged content-addressed data assets and write only changed texture targets. GPU capture uses asynchronous readback and compression runs away from the editor thread; Unity's final asset import is synchronous and can still cause a short editor hitch. Closing while the document is modified or a matching recovery asset exists offers **Save**, **Discard**, and **Cancel**. Save commits the project document before deleting recovery, Discard deletes recovery and closes without saving, and Cancel leaves both the editor session and recovery untouched. If a background autosave is already running when Close is requested, the window closes immediately and that in-flight save becomes the final close-save; the stage exits automatically after its durable commit. Export completion restarts the configured autosave debounce so a long asset import cannot unexpectedly begin a document capture while the export and painter windows are being closed.
...
The document losslessly persists editable base pixels, layer pixels, masks, splines, per-layer source/brush settings, and workspace state. Surfaces bind through stable material/slot/mesh identities rather than temporary reconstruction indexes. UMA's random `_Genb_<number>` generated-material nonce is excluded from new surface identities. Existing documents whose legacy ids include that nonce rebind through a conservative score of slot ownership, UV/topology fingerprints, UMA material, and the saved renderer/submesh location; an unchanged UV layout restores raster layers and masks exactly even when regenerated triangle ordering differs. Export reads either the live flattened composite or a temporary authored-layers-only composite, according to the selected export mode, without saving or changing the document, avatar, recipe, or scene. Recovery assets are temporary project content and should not be shipped. To keep the default recovery location local, add both `/Assets/UMA/Temp/` and `/Assets/UMA/Temp.meta` to the project's source-control ignore file. If the recovery folder setting is changed, ignore the configured folder and its `.meta` file instead.
...
Documents fingerprint geometry, topology, UVs, and material bindings independently. Stable matches restore exactly; compatible material or reconstruction changes rebind automatically. Rebinding prefers exact UV plus topology, then unchanged UV, matching topology, and finally a matching saved renderer/submesh backed by compatible material evidence; slot overlap is always required. When topology still matches but UVs change, surface-anchored strokes and spline paths are retained for rerasterization while stale raster pixels are preserved as explicitly orphaned document content. Layer masks retain their base value, grayscale paint value, and procedural effects but reset stale pixel-space painting to the base value. Removed or incompatible surfaces are reported in the export window instead of being silently discarded or painted onto the wrong mesh.
