...
{
    public class UMARemapDrawer : MaterialPropertyDrawer
    {
        private Vector2 range = Vector2.up;

        public UMARemapDrawer() { }

        public UMARemapDrawer(float min, float max)
        {
            this.range.x = min;
            this.range.y = max;
        }

        public UMARemapDrawer(float min, float max, float negativeOffset)
        {
