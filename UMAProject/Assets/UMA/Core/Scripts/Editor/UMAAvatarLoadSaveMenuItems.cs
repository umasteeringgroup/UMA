...
namespace UMA.Editors
{
	public class UMAAvatarLoadSaveMenuItems : Editor
	{
			...
			for (int i = 0; i < Selection.gameObjects.Length; i++)
			{
				...
				if (avatar != null)
				{
					...
					if (path.Length != 0)
					{
						...
						{
							asset.Save(avatar.umaData.umaRecipe, avatar.context, (avatar as UMA.CharacterSystem.DynamicCharacterAvatar).WardrobeRecipes, true);
						}
						else
						{
			...
			for (int i = 0; i < Selection.gameObjects.Length; i++)
			{
				...
				if (avatar != null)
				{
					...
					if (path.Length != 0)
					{
						...
						if (avatar is UMA.CharacterSystem.DynamicCharacterAvatar)
						{
							(avatar as UMA.CharacterSystem.DynamicCharacterAvatar).LoadFromRecipeString(asset.recipeString);
						}
						else
						{
							avatar.Load(asset);
				...
				if (avatar != null)
				{
					...
					if (path.Length != 0)
					{
						...
						if (asset != null)
						{
							...
							if (avatar is UMA.CharacterSystem.DynamicCharacterAvatar)
							{
								(avatar as UMA.CharacterSystem.DynamicCharacterAvatar).LoadFromRecipe(asset);
							}
							else
							{
								avatar.Load(asset);
		...
		public static void SaveSelectedAvatarsDCSTxt()
		{
			...
			{
				var selectedTransform = Selection.gameObjects[i].transform;
				var avatar = selectedTransform.GetComponent<UMA.CharacterSystem.DynamicCharacterAvatar>();
