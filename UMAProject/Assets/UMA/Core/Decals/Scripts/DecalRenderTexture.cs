...
namespace UMA {
 ...
 DecalRTOptions options = null) {
			 ...
			 for (int v = start; v < end; v++)
			 {
				...
				foreach(var kv in slotTriMap) {
					...
					for(int gi = 0; gi < umaData.generatedMaterials.materials.Count; gi++) {
						...
						if(gm == null)
							continue;
						if(gm.skinnedMeshRenderer != smr)
							continue;
						if(gm.umaMaterial == null || !gm.umaMaterial.Equals(slot.material))
							continue;
						if(gm.resultingAtlasList == null)
							continue;

						if(options.enableDebug) {
							LogInfo($"Found GeneratedMaterial: mat='{gm.material?.name}', channels={gm.resultingAtlasList.Length}.");
						}
						...
						for(int ch = 0; ch < channels; ch++) {
							...
							if(options.enableDebug)
								LogTextureInfo($"Source channel[{ch}]", src);
							if(src == null)
								continue;
							var tgtTex = gm.resultingAtlasList[ch];
							if(options.enableDebug)
								LogTextureInfo($"Target channel[{ch}] BEFORE", tgtTex);
							if(tgtTex == null)
								continue;
							...
							if(rt == null) {
								...
								if(options.enableDebug) {
									LogInfo($"Created temp RT '{rt.name}' from Texture2D size {w}x{h}.");
									LogTextureInfo("Temp RT", rt);
								}
							...
							if(expandedMesh == null) {
								LogWarn($"Expanded mesh is null for channel {ch}. Skipping draw.");
								continue;
							}
							...
							if(options.enableDebug) {
								LogRTActive($"Before DrawMeshNow ch[{ch}]");
							}
							try {
								stampMat.SetPass(0);
								SaveRenderTexturePNG(rt, stampAsset, stampAsset.overlayName, Time.frameCount, "Before DrawMeshNow");
								Graphics.DrawMeshNow(expandedMesh, Matrix4x4.identity);
								SaveRenderTexturePNG(rt, stampAsset, stampAsset.overlayName, Time.frameCount, "After DrawMeshNow");
								GL.Flush();
								SaveRenderTexturePNG(rt, stampAsset, stampAsset.overlayName, Time.frameCount, "after GL.Flush");
								stampedCount++;
							} catch(Exception ex) {
								LogError($"DrawMeshNow exception: {ex.Message}\n{ex.StackTrace}");
							} finally {
							...
							if(options.bleedPixels > 0) {
								if(options.enableDebug)
									LogInfo($"Running dilation: {options.bleedPixels} px on '{rt.name}'.");
							...
							if(createdTempRT) {
								...
								if(options.enableDebug) {
									LogTextureInfo($"Baked Texture2D ch[{ch}]", newTex);
								}
								...
								LogInfo($"Rebinding baked texture to material '{gm.material?.name}', channel {ch}, property '{propName}'.");
							...
							if(options.enableDebug) {
								LogTextureInfo($"Target channel[{ch}] AFTER", gm.resultingAtlasList[ch]);
							}
						} // end for ch
					} // end for gi
		...
		public static bool ApplyStampToUMA(DynamicCharacterAvatar avatar, UMAData umaData, DecalRTStampAsset stamp, string materialPropertyName, RenderTexture targetTexture) {
			...
			LogInfo($"ApplyStampToUMA: Begin. overlay='{stamp.overlayName}', slots={stamp.slots?.Count ?? 0}. ColorSpace={QualitySettings.activeColorSpace}");
			LogTextureInfo("Apply target RT", targetTexture);
			...
			if(maskTex != null) {
				stampMat.SetTexture("_MaskTex", maskTex);
				LogTextureInfo("Apply maskTex", maskTex);
			}

			var prevRTGlobal = RenderTexture.active;
			GL.PushMatrix();
			GL.LoadOrtho();
			LogRTActive("ApplyStampToUMA: Before stamping");
			...
			for(int si = 0; si < stamp.slots.Count; si++) {
                ...
                // Always map normalized UVs (0..1) from recorded area into current slot UVArea
                bool recValid = s.recordedUVArea.width > 0f && s.recordedUVArea.height > 0f;
                bool curValid = slot.UVArea.width > 0f && slot.UVArea.height > 0f;
				...
				for(int v = 0; v < vcount; v++) {
					Vector2 nuv = s.normBaseUV[v];
                    // First convert normalized -> atlas UV using recorded area
                    Vector2 atlasUVRecorded = recValid
                        ? new Vector2(
                            s.recordedUVArea.xMin + nuv.x * s.recordedUVArea.width,
                            s.recordedUVArea.yMin + nuv.y * s.recordedUVArea.height)
                        : nuv; // fallback full atlas
                    // Then convert to normalized relative to current UVArea
                    Vector2 normCurrent = curValid
                        ? new Vector2(
                            (atlasUVRecorded.x - slot.UVArea.xMin) / slot.UVArea.width,
                            (atlasUVRecorded.y - slot.UVArea.yMin) / slot.UVArea.height)
                        : atlasUVRecorded;
                    // And finally to atlas UV based on current area
                    Vector2 globalUV = curValid
                        ? slot.ConvertToAtlasUV(new Vector2(Mathf.Clamp01(normCurrent.x), Mathf.Clamp01(normCurrent.y)))
                        : normCurrent;
                    uv0List.Add(globalUV);
					var ov = s.overlayUV[v];
				...
				LogTextureInfo("ApplySlot srcTex", texToStamp);
				...
				LogRTActive("ApplyStampToUMA: Before DrawMeshNow");
			...
			GL.PopMatrix();
			RenderTexture.active = prevRTGlobal;
			LogRTActive("ApplyStampToUMA: After stamping");
		...
		RenderTexture targetTexture, int srcOverlayNameHash) 
		{
			...
			try {
				...
				//SaveRenderTexturePNG(targetTexture, stamp, stamp.overlayName, Time.frameCount, "ApplySlotStamps_Start");
