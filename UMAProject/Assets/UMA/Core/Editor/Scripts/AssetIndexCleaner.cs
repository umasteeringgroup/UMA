...
{
    /// <summary>
    /// Removes every user-added type from the UMA asset index and rebuilds all
    /// serialized and runtime lookups using only UMA's standard indexed types.
    /// </summary>
    public static class AssetIndexCleaner
    {
        [MenuItem("UMA/Cleanup Types")]
        private static void CleanupTypes()
        {
            if (IsGlobalLibraryWindowOpen())
            {
                EditorUtility.DisplayDialog(
                    "Close the UMA Global Library",
                    "The UMA Global Library window is currently open. Close it before cleaning the asset index, " +
                    "then run UMA > Cleanup Types again.",
                    "OK");
                return;
            }
            ...
            if (!EditorUtility.DisplayDialog(
                    ...
                    "This removes all added index types and every indexed item belonging to those types. " +
                    "Only UMA's standard index types and their items will remain.\n\nThis operation cannot be undone.",
                    "Cleanup Types",
                    "Cancel"))
            {
                return;
            ...
            if (indexer == null)
            {
                Debug.LogError("[UMA] Cleanup Types could not load the UMAAssetIndexer.");
                return;
            }
            ...
            try
            {
                ...
                string summary =
                    $"[UMA] Asset index type cleanup complete. Removed {result.AddedTypesRemoved} added type(s), " +
                    $"{result.SerializedItemsRemoved} serialized item(s), {result.IndexedTypeNamesRemoved} persisted " +
                    $"type name(s), and {result.TypeFolderEntriesRemoved} orphaned type-folder entry/entries. " +
                    $"{result.SerializedItemsRemaining} serialized item(s) remain across UMA's {result.MainTypesRemaining} standard type(s).";

                Debug.Log(summary, indexer);
                EditorUtility.DisplayDialog(
                    "UMA Asset Index Cleanup Complete",
                    summary,
                    "OK");
            }
            catch (Exception exception)
            {
                ...
                EditorUtility.DisplayDialog(
                    ...
                    "The UMA asset index could not be cleaned. See the Console for details.",
                    "OK");
            }
        }

        [MenuItem("UMA/Cleanup Types", true)]
        private static bool ValidateCleanupTypes()
        {
            return !Application.isPlaying && !EditorApplication.isCompiling;
        }

        private static bool IsGlobalLibraryWindowOpen()
        {
