...
namespace UMA.CharacterSystem
{
    ...
    public class DynamicCharacterAvatar : UMAAvatarBase
    {
        ...
        public static long Ticks_LoadPhase5 = 0;
...
#region METHODS 
        ...
        public void SetRenderers(bool val)
        {
            ...
            if (RendererCount > 0)
            {
                ...
                if (frenderer != null)
                {
                    ...
                    {
                        SkinnedMeshRenderer[] array = GetRenderers();
                ...
                else
                {
                    if (leanHiding)
                    {
                        ...
                        {
                            Dirty(false, true, false);
                            OnCharacterUpdated += UnhideRenderers;
                        }
                        else
                        {
                            // In the editor, we need to unhide immediately.
                            UnhideRenderers(this);
                        }
                    }
                    else
                    {
        ...
        public void ForceWear(UMATextRecipe utr)
        {
            internalSetSlot(utr, utr.wardrobeSlot);
        }
        ...
        private void TryImportDNAValues(UMADnaBase[] prevDna)
        {
            ...
            var activeDNA = umaRecipe.GetAllDna();
        ...
        private Dictionary<string, float> GetDefaultDNA(UMAData.UMARecipe recipe)
        {
            Dictionary<string, float> dna = new Dictionary<string, float>();
            if (activeRace == null)
            {
        ...
        public Dictionary<string, DnaSetter> GetDNA(UMAData.UMARecipe recipe = null)
        {
            ...
            if (activeRace.racedata.useNewDNA)
            {
                ...
                foreach (var kvp in DNAbyGroup)
                {
                    ...
                    foreach (var dnaInstance in instances)
                    {
   ...
   thisDCASA|= SaveOptions.saveAnimator;*/
        ...
        public void BuildCharacter(bool RestoreDNA = true, bool skipBundleCheck = false, bool useBundleParameter = true, bool forceBuild=false)
        {
            ...
            UMADnaBase[] CurrentDNA = null;
                ...
                if (umaRecipe != null)
                {
                    CurrentDNA = umaRecipe.GetDefinedDna();
                }
            ...
            if ((this.WardrobeRecipes.Count > 0 || this._additiveRecipes.Count > 0) && activeRace.racedata != null)
            {
                ...
                for (int i = 0; i < allRecipes.Count; i++)
                {
                    ...
                    if (utr.suppressWardrobeSlots != null && utr.suppressWardrobeSlots.Count > 0)
                    {
                        // Race compat check (unchanged logic)
                        if (activeRace.name == "" ||
                            utr.compatibleRaces.Count == 0 ||
                            utr.compatibleRaces.Contains(activeRace.name) ||
                        ...
                        {
                            for (int s = 0; s < utr.suppressWardrobeSlots.Count; s++)
                            {
                                string suppressedSlot = utr.suppressWardrobeSlots[s];
                                if (!SuppressSlotsStrings.Contains(suppressedSlot))
                ...
                // Pass 2: per race wardrobe slot ordering (unchanged logic)
                for (int i = 0; i < activeRace.racedata.wardrobeSlots.Count; i++)
                {
                    string ws = activeRace.racedata.wardrobeSlots[i];

                    if (SuppressSlotsStrings.Contains(ws))
                    {
                        if (this.WardrobeRecipes.ContainsKey(ws))
                        {
                            SuppressedRecipes.Add(this.WardrobeRecipes[ws]);
                        }
                        continue;
                    }
                    ...
                    if (this.WardrobeRecipes.ContainsKey(ws))
                    {
                        UMATextRecipe utr = this.WardrobeRecipes[ws];
                        if (utr != null && !utr.disabled)
                        {
                            UMAWardrobeRecipe umr = (utr as UMAWardrobeRecipe);
                            bool utrIsCross = (activeRace.racedata.IsCrossCompatibleWith(utr.compatibleRaces) &&
                                               activeRace.racedata.wardrobeSlots.Contains(utr.wardrobeSlot));
                            if (utrIsCross)
                            {
                                wasCrossCompatibleBuild = true;
                                for (int c = 0; c < utr.compatibleRaces.Count; c++)
                                {
                                    var cr = utr.compatibleRaces[c];
                                    if (!crossCompatibleRaces.Contains(cr))
                                        crossCompatibleRaces.Add(cr);
                                }
                            }
                            ...
                            {
                                for (int h = 0; h < utr.Hides.Count; h++)
                                {
                                    string hid = utr.Hides[h];
                                    HiddenSlots.Add(hid);
                                    if (utrIsCross)
                                    {
                                        var eq = activeRace.racedata.FindEquivalentSlot(utr.compatibleRaces, hid, false);
            ...
            tm.Stop();
            Ticks_Phase2 += tm.ElapsedTicks;
            tm.Restart();
            // Additional recipes (unchanged, original logic below)
...
#endif
            tm.Stop();
            Ticks_Phase4 += tm.ElapsedTicks;
            LoadCharacter(serializedRecipe, ReplaceRecipes, WardrobeRecipes, umaAdditionalRecipes, MeshHideDictionary, HiddenSlots, HideTags, CurrentDNA, RestoreDNA, skipBundleCheck);
#if UMA_DCA_TIMING
            sw.Stop();
        ...
        private void LoadCharacter(UMARecipeBase baseRaceRecipe, List<UMAWardrobeRecipe> Replaces, List<UMATextRecipe> wardrobeRecipes, UMARecipeBase[] AdditionalRecipes, Dictionary<string, List<MeshHideAsset>> MeshHideDictionary, List<string> hiddenSlots, List<string> HideTags, UMADnaBase[] CurrentDNA, bool restoreDNA, bool skipBundleCheck)
        {
           ...
           /* if (Application.isEditor && !Application.isPlaying)
            ...
            AddAdditionalRecipes(AdditionalRecipes, false);
            if (wardrobeRecipes != null)
            {
                AddWardrobeRecipes(wardrobeRecipes);
            }
            tm.Stop();
            Ticks_LoadPhase3 += tm.ElapsedTicks;
            tm.Restart();
            ...
            for (int i = 0; i < umaRecipe.slotDataList.Length; i++)
            {
                ...
                {
                    smooshSlots.Add(sd);
                    if (VertexOverrides.ContainsKey(sd.slotName))
                    {
                        VertexOverrides.Remove(sd.slotName);
                    }
                }
            }
            ...
            for (int i = 0; i < clippingPlanes.Count; i++)
            {
                ...
                {
                    SlotData lookSloot = umaRecipe.slotDataList[i1];
     ...
     /*   private void AddMeshModifiers(UMARecipeBase baseRaceRecipe, List<UMAWardrobeRecipe> Replaces, List<UMARecipeBase> wardrobeRecipes, UMARecipeBase[] AdditionalRecipes)
        ...
        public void SmooshSlotPhysics(UMAData umaData, SlotDataAsset SmooshMe, SlotDataAsset SmooshPlane, SlotDataAsset SmooshTarget, bool invertX, bool invertY, bool invertZ, bool invertDist, float smooshDistance, float overSmoosh)
        {
            ...
            try
            {
                ...
                if (VertexOverrides.ContainsKey(SmooshMe.slotName))
                {
                    sourceVertexes = VertexOverrides[SmooshMe.slotName];
                }
...
#endif
                AddVertexOverride(SmooshMe, newVerts);
            }
            finally
            {
                CleanScene(SmooshScene);
        ...
        private void UnloadAvatar()
        {
            ...
            serializedRecipe = null;
            umaRecipe.ClearDna();
            umaRecipe.SetSlots(new SlotData[0]);
            umaRecipe.sharedColors = new OverlayColorData[0];
        ...
        public void AddWardrobeRecipes(List<UMATextRecipe> wardrobeRecipes)
        {
            if (wardrobeRecipes != null)
            {
                for (int i = 0; i < wardrobeRecipes.Count; i++)
                {
                    UMARecipeBase umaAdditionalRecipe = wardrobeRecipes[i];
                    if (umaAdditionalRecipe != null)
                    {
                        UMARecipe cachedRecipe = umaAdditionalRecipe.GetCachedRecipe();
                        umaRecipe.Merge(cachedRecipe, false, true, false, activeRace.racedata.raceName);
                    }
#if UNITY_EDITOR
                    else
                    {
        ...
        {
            var recipeSlots = umaRecipe.slotDataList;
        ...
        void OldReplaceSlot(UMAWardrobeRecipe Replacer)
        {
            ...
            for (int i = 0; i < umaRecipe.slotDataList.Length; i++)
            {
                SlotData originalSlot = umaRecipe.slotDataList[i];
                ...
                if (originalSlot.slotName == replaceSlot)
                {
                    ...
                    UMARecipe TempRecipe = UMATextRecipe.UnpackRecipe(PackRecipe);
                    if (TempRecipe.slotDataList.Length > 0)
                    {
                        ...
                        for (int i1 = 0; i1 < TempRecipe.slotDataList.Length; i1++)
                        {
                            ...
                            if (replacementSlot != null)
                            {
                                ...
                                umaRecipe.slotDataList[i] = replacementSlot;
                                break;
                            }
                        }
                    }
        ...
        bool ReplaceSlot(UMAWardrobeRecipe Replacer)
        {
            ...
            for (int i = 0; i < umaRecipe.slotDataList.Length; i++)
            {
                ...
                if (originalSlot.slotName == replaceSlot)
                {
                    ...
                    if (newSlot != null)
                    {
                        ...
                        umaRecipe.slotDataList[i] = newSlot;
                        replaced = true;
                    }
                }
            }
            ...
            if (ReplacedOverlays != null)
            {
                for (int i = 0; i < umaRecipe.slotDataList.Length; i++)
                {
                    SlotData checkSlot = umaRecipe.slotDataList[i];
                    if (checkSlot == null)
                    {
                        continue;
                    }
        ...
        void PostProcessSlots(List<string> hiddenSlots, List<string> hideTags = null)
        {
            ...
            umaRecipe.slotDataList = NewSlots.ToArray();
        ...
        public class WardrobeRecipeList
        {
            ...
            public List<WardrobeRecipeListItem> GetRecipesForRace(string raceName = "", RaceData race = null)
            {
                ...
                for (int i = 0; i < recipes.Count; i++)
                {
                    ...
                    if (WLIRecipe.ForceLoad || raceName == "" || WLIRecipe._recipe.compatibleRaces.Contains(raceName) || (race != null && race.IsCrossCompatibleWith(WLIRecipe._recipe.compatibleRaces)))
                    {
