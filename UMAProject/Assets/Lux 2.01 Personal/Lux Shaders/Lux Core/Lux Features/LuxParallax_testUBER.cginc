...
float2 hgtDDX = dx * _ParallaxMap_TexelSize.zw;
float2 hgtDDY = dy * _ParallaxMap_TexelSize.zw;
...
float delta=_ParallaxMap_TexelSize.x*exp2(mip);

//fStepSize *= delta/length(-viewDir.xy);
...
// super wichtig!
// _Parallax oder fParallaxLimit macht keinen unterschied
float EyeDirTanZ = viewDir.z / _Parallax; // fParallaxLimit; //// POM_Linear_Steps; // //max(0.001, _Parallax);
// float scl=dh_prev / ((_h-h_prev) - EyeDirTan.z);
// _H is current sampled height

// das ist unser delta, unseer rhamen in y space, der zu untersuchen ist: irgendwo hier!
float dh_prev = (fCurrRayHeight - fLastSampledHeight); //fCurrRayHeight - fStepSize); // );
// secantLine ist steigung!!!!
float secantLine = dh_prev / (( fCurrSampledHeight - fLastSampledHeight ) - EyeDirTanZ );
float4 newOffset = vCurrOffset - finalStepSize * (1 - secantLine); // back

fCurrRayHeight -= fStepSize * (1 - secantLine); // back

heightAndMask = tex2Dgrad(heightmap, uvScaled.xy + newOffset.xy, dx.xy, dy.xy).gbr;
h0 = heightAndMask.x;
h1 = tex2Dgrad(heightmap, uvScaled.zw + newOffset.zw, dx.zw, dy.zw).a;
//#if defined(GEOM_TYPE_LEAF)
//mixmapValue = half2(heightAndMask.y, 1.0 - heightAndMask.y);
//            mixmapValue = max( half2(0.0001, 0.0001), mixmapValue * float2(dot(h0, mixmapValue.x), dot(h1, mixmapValue.y)));
            // one might skip it in the loop and do it at the end
...
float newSampledHeight = lerp(h0, h1, mixmapValue.y);
float3 lastStep = float3(finalStepSize.xy, EyeDirTanZ);
if (newSampledHeight >= fCurrRayHeight) {
    lastStep *= secantLine;
    secantLine = dh_prev / (( newSampledHeight - fLastSampledHeight ) - lastStep.z );
    vCurrOffset -= lastStep.xyxy * (1 - secantLine);
}
else {
    lastStep *= (1 - secantLine); 
    secantLine = dh_prev / (( fCurrSampledHeight - newSampledHeight ) - lastStep.z );
    vCurrOffset += lastStep.xyxy * (secantLine); 
}
...
float4 newOffset = vCurrOffset - fCurrSampledHeight.xxxx * (vCurrOffset - vLastOffset ) / (fCurrSampledHeight.xxxx - fLastSampledHeight.xxxx ) - EyeDirTanZ;
// muss ich die rayheight nehmen?
//  newOffset = vCurrOffset - fCurrRayHeight.xxxx * (vCurrOffset - vLastOffset ) / (fCurrRayHeight.xxxx - fLastSampledHeight.xxxx ); // + EyeDirTanZ;
newOffset = vCurrOffset - (fCurrSampledHeight.xxxx - fCurrRayHeight.xxxx) * (vCurrOffset - vLastOffset ) / (fCurrSampledHeight.xxxx - fLastSampledHeight.xxxx ); // -  EyeDirTanZ;

float secantLine = (fCurrRayHeight - fCurrSampledHeight) / ( (fCurrSampledHeight - fLastSampledHeight ) - EyeDirTanZ);
newOffset = vCurrOffset - finalStepSize * (secantLine);
fCurrRayHeight -= fStepSize * (1 - secantLine); // back


float fdelta1 = fCurrSampledHeight - fCurrRayHeight;
            float fdelta2 = (fCurrRayHeight + fStepSize) - fLastSampledHeight;
            float fratio = fdelta1/(fdelta1+fdelta2);
newOffset = (fratio) * vLastOffset + (1.0-fratio) * vCurrOffset;
...
//fCurrRayHeight -= fStepSize * (newOffset.x - vCurrOffset.x);

//vCurrOffset = newOffset;

                    //fCurrRayHeight = (fratio) * (fCurrRayHeight + fStepSize) + (1.0-fratio) * fCurrRayHeight;

if (newSampledHeight >= fCurrRayHeight) {
    newOffset += (newOffset - vCurrOffset) * 0.5; // lastStep.xyxy * (1 - secantLine);
}
else {
  newOffset -= (newOffset - vCurrOffset) * 0.5; //lastStep.xyxy * (1 - secantLine);   
}
...
// Calculate height according to mixmapValue
        ...
        if (newSampledHeight >= (fCurrRayHeight + fStepSize * ( 1-ratio)) ) {
//            vCurrOffset -= finalStepSize * (1-ratio) * 0.5;
        }
        else {
//           vCurrOffset += finalStepSize * (ratio) * 0.5;
        }
...
//    fParallaxLimit *= _Parallax; // * detailBlendState;
    ...
    // Calculate the parallax offset vector direction and maximum offset.
    float2 vOffsetDir = viewDir.xy; //normalize(viewDir.xy);
    ...
    float3 Step = float3(vOffsetDir, viewDir.z) * fParallaxLimit * float3(_Parallax.xx, 1) * (1.0 / (float)POM_Linear_Steps);
    //Step.z /= _Parallax;
    ...
    float CurrentSampledSecantHeight = 1.0;
    ...
    for (int i = 0; i < POM_Linear_Steps; i++, CurrentUVs_and_Depth.xyz += Step) {
         heightAndMask = tex2Dgrad(heightmap, CurrentUVs_and_Depth.xy, dx.xy, dy.xy).gbr;
         CurrentSampledHeight = heightAndMask.g;
    ...
    if (SampleIsBelowRay) {
        ...
        heightAndMask = tex2Dgrad(heightmap, CurrentUVs_and_Depth.xy, dx.xy, dy.xy).gbr;
        CurrentSampledSecantHeight = heightAndMask.g;

        // Step forwards
        if (CurrentSampledSecantHeight <= CurrentUVs_and_Depth.z) {
            Step *= slope;
            LastOffset = CurrentUVs_and_Depth.z - CurrentSampledSecantHeight;
            slope = LastOffset / ((CurrentSampledHeight - CurrentSampledSecantHeight) - Step.z);
        ...
        // Step backwards
            ...
            Step *= 1.0 - slope;
            slope = 1.0 - LastOffset / ((CurrentSampledSecantHeight - LastSampledHeight) - Step.z);
            CurrentUVs_and_Depth.xyz -= Step * slope;
        }
...
//    }
    ...
    // Set offset
    uvIN = CurrentUVs_and_Depth.xyxy; 
